...
// require_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'FileDataSource.php';
...
class NBC {
    ...
    private function posterior($tokens, $class) {
        ...
        foreach ($tokens as $token) {
            $count = isset($this->vocabulary[$token][$class]) ? $this->vocabulary[$token][$class] : 0;   
    ...
    private function prior($class) {
        return $this->documentCounter == 0 ? 0 : ($this->classDocumentCounter[$class] / $this->documentCounter);
