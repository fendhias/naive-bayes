...
{
  public function preprocesing($from,$to){
        // echo "string".$from;
        // echo "string".$to;
        // die();
        $terms = array();
      ...
      $tweets = DB::table('tweets')
                    ->whereBetween('date_tweet',[$from,$to])->get();
    foreach ($tweets as $tweet) {
            if(strtotime($tweet->date_tweet) >= strtotime($from) && strtotime($tweet->date_tweet) <= strtotime($to))
            {
          ...
          { 
                ...
                if($pos_tagger->tweet_new){
                    DB::table('tweet_preprocessing')->insert(['id_tweet' => $tweet->id, 'username' => $tweet->username,'preprocessing' => rtrim($clear_tweet,' ')]);
    ...
    public function cleansing($term)
    {
